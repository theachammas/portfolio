...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!document.querySelector('nav')) {
    ...
    const homeLink = inProjectsFolder ? '../index.html' : 'index.html';
  ...
  // Thumbnails now link directly to project pages - no modal needed!
  ...
  if (greater) {
    // Make it clickable and link to first project
    greater.style.cursor = 'pointer';
    greater.style.transition = 'color 0.3s ease, transform 0.3s ease';
    
    greater.addEventListener('click', function() {
      // Turn red and navigate
      this.style.color = '#ff0000';
      this.style.transform = 'rotate(180deg)';
      
      setTimeout(() => {
        window.location.href = 'projects/optical-illusion.html';
      }, 300);
    });
    
    // Hover effect
    greater.addEventListener('mouseenter', function() {
      this.style.color = '#ff0000';
    });
    
    greater.addEventListener('mouseleave', function() {
